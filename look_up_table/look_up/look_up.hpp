...
class look_up {
	public:
		look_up(std::string file_name, std::function<T(T)> func, const double epsilon = 0.01f, const double lim_min = 0.0f, const double lim_max = 100.0f) {
			file_name = "../../PFE/Robot/Controller/"+file_name+".hpp";
			std::ofstream file = std::ofstream(file_name);

			file << "namespace lut {\n	const double lim_min = " << static_cast<T>(lim_min) << ", lim_max = " << static_cast<T>(lim_max) << ", epsilon = " << static_cast<T>(1.0f/epsilon) << ";\n";
