...
Dsensor = table2array(readtable(strcat('DVS_points_',Nfile,'.csv')));
Dtracker = table2array(readtable(strcat('Cluster_points_',Nfile,'.csv')));
...
figure(2);
hold on;
plot3(Dtracker(:,1),Dtracker(:,2),Dtracker(:,4)-Ttime(:,1)*ones(length(Dtracker),1),'r*');
plot3(Dsensor(:,1),Dsensor(:,2),Dsensor(:,4)-Ttime(:,1)*ones(length(Dsensor),1),'k*');
view(3);
hold off;
