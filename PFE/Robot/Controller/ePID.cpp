...
ePID::ePID(std::chrono::time_point<std::chrono::high_resolution_clock> begin_timestamp, const int num_file, const float Kp, const float Ki, const float Kd, const float N, const unsigned int nb_corrector, const float e_lim, const float hnom, const float alpha_i, const float alpha_d)
	: BaseThread("ePID"), m_kp(Kp), m_ki(Ki/2), m_kdN(Kd*N), m_nb_corrector(nb_corrector), m_elim(e_lim), m_hnom(hnom), m_alpha_i(alpha_i), m_alpha_d(alpha_d), m_begin_timestamp(begin_timestamp) {
	...
	if(LENGTH_PID_CHAIN == m_nb_corrector+1) {
		m_logCPUhard = new logger("hard_timing"+std::to_string(m_nb_corrector), begin_timestamp, num_file);
	}
...
void* ePID::ThreadRun() {
	while(GetStartValue()) {
		...
		if(std::fabs(g_setpoint[m_nb_corrector].load()-g_feedback[m_nb_corrector].load()) > m_elim) {
			bool b = false;
			if(g_event[m_nb_corrector].load()) {b = true;}
			ComputePID();
			if(b) {g_event[m_nb_corrector].store(false);}
		} else if(g_event[m_nb_corrector].load()) {
			ComputePID();
